...
class Encoder(nn.Module):
    ...
    def __init__(self, L, N):
        ...
        self.conv1d_U_2 = nn.Conv1d(N, 2 * N, kernel_size=L // 2, stride=L // 4, bias=False)

        self.lstm = nn.LSTM(330, 330, batch_first=True)
    ...
    def forward(self, mixture):
        ...
        x = self.conv1d_U(mixture)
        x = self.pool1(x)
        x = self.conv1d_U_2(x)
        mixture_w,_ = self.lstm(x)  # [M, N, K]
        return mixture_w
...
class Decoder(nn.Module):
    def __init__(self, N, L):
        ...
        self.N, self.L = N, L
        # Components
        self.basis_signals = nn.Linear(2*N, L, bias=False)
...
class TemporalConvNet(nn.Module):
    def __init__(self, N, B, H, P, X, R, C, norm_type="gLN", causal=False,
        ...
        # Components
        # [M, N, K] -> [M, N, K]
        layer_norm = ChannelwiseLayerNorm(N*2)
        # [M, N, K] -> [M, B, K]
        ...
        # [M, B, K] -> [M, C*N, K]
        mask_conv1x1 = nn.Conv1d(B, C*N*2, 1, bias=False)
        # Put together
