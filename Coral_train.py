...
from solver import Solver,Solver_CORAL
from conv_tasnet import ConvTasNet, ConvTasNet_Coral
...
def main(args):
    ...
    mixitem_source = np.load('/mnt/denoisedata/eegnoise_3mix_3.npy')
    eeglabels = np.load('/mnt/denoisedata/eeglabels_3mix_3.npy')
    mixitem_target = np.load('/mnt/denoisedata/ObjectEEG.npy')
    mixitem_target = np.repeat(mixitem_target, 4, axis=0)

    X_test_source = mixitem_source[432:648]
    X_test_target = mixitem_target[432:648]
    y_test = eeglabels[432:648]
    X_train_source = mixitem_source[648:mixitem_target.shape[0]]
    X_train_target = mixitem_target[648:mixitem_target.shape[0]]
    y_train = eeglabels[648:mixitem_target.shape[0]]

    X_train_source = torch.tensor(X_train_source, dtype=torch.float32)
    X_train_target = torch.tensor(X_train_target, dtype=torch.float32)
    y_train = torch.tensor(y_train, dtype=torch.float32)
    X_test_source = torch.tensor(X_test_source, dtype=torch.float32)
    X_test_target = torch.tensor(X_test_target, dtype=torch.float32)
    y_test = torch.tensor(y_test, dtype=torch.float32)


    X_train_source = X_train_source.to(device)
    X_train_target = X_train_target.to(device)
    y_train = y_train.to(device)
    X_test_source = X_test_source.to(device)
    X_test_target = X_test_target.to(device)
    y_test = y_test.to(device)

    batch_size = 200
    # 创建数据集和数据加载器
    train_dataset = TensorDataset(X_train_source,X_train_target,y_train)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True)

    test_dataset = TensorDataset(X_test_source,X_test_target, y_test)
    ...
    # model
    model = ConvTasNet_Coral(args.N, args.L, args.B, args.H, args.P, args.X, args.R,
    ...
    # solver
    solver = Solver_CORAL(data, model, optimizier, args)
    solver.train()
